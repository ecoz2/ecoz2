...
- initial parallelized version of `vq_learn` using openmp.
  Again, conditioned by a PAR define in the source to facilitate
  testing alternating between the parallel and serialized versions.
  Only parallelizing the inner loop related with getting the minimum
  distortion.
  Results:
     PAR: ~193s; SER: ~683s,
     so 3.5x speedup.

        $ vq.learn -P 36 -e 0.0005 data/predictors/data/MARS_20161221_000046_SongSession_16kHz_HPF5Hz.prd

    Did inspection of results by visually comparing the report (`eps_0.0005.rpt`)
    in terms of cardinalities, distortions, etc, between the serialized
    and parallel runs noting them reasonable, consistent.
    The seed in both case was random as the distribution of work across
    the threads would also be random.
    Some future experiment is to use some of the "schedule" options along
    with a fixed seed in both version to then perform a direct numerical  
    comparison.
